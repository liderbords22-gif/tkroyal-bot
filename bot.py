...
(REGISTER, CHOOSE_ACTION, MARCH_ORIGIN_CITY, MARCH_ORIGIN_COUNTRY, MARCH_DEST_CITY, MARCH_DEST_COUNTRY, MARCH_TYPE, MARCH_TIME, MARCH_STATS, WAR_ORIGIN_CITY, WAR_ORIGIN_COUNTRY, WAR_DEST, WAR_STATS, DECLARE_TEXT, AMBUSH_STATS, CHANGE_DEST, CHANGE_TIME) = range(17)
...
async def march_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    channel_msg = (f"⚔️ *لشکرکشی*\n━━━━━━━━━━━━━━━\n"
        ...
        f"📋 نوع: {d['mt']}\n"
        f"🕐 زمان رسیدن: {d['m_time']}\n━━━━━━━━━━━━━━━\n"
        f"🗓 {now}")

    kb = InlineKeyboardMarkup([[InlineKeyboardButton("🏹 کمین", url=f"https://t.me/ThreeKingdomsRoyal_bot?start=ambush"), InlineKeyboardButton("🔙 بک", callback_data=f"back_{user.id}"), InlineKeyboardButton("🔀 تغییر مسیر", callback_data=f"change_{user.id}")]])

    msg = await context.bot.send_message(CHANNEL_ID, channel_msg, parse_mode="Markdown", reply_markup=kb)
    context.user_data["channel_msg_id"] = msg.message_id
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("back_"):
        ...
        for line in original.split("\n"):
            if "ارتش" in line:
                country = line.split("*")[1] if "*" in line else ""
        await context.bot.send_message(CHANNEL_ID, f"🔙 *بازگشت*\n━━━━━━━━━━━━━━━\nلشکر *{country}* بازگشت\n🗓 {now}", parse_mode="Markdown")
        await query.edit_message_reply_markup(reply_markup=None)

    elif data.startswith("change_"):
        context.user_data["change_msg_id"] = query.message.message_id
        context.user_data["change_original"] = query.message.text
        await context.bot.send_message(int(data.split("_")[1]), "🔀 مقصد جدید را وارد کنید:")
        context.user_data["waiting_change"] = True
